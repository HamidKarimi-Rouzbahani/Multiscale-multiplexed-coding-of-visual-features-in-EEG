...
for i=1:size(accuracies,2)
    ...
    for t=1:length(Effects)
        if Effects(t)>=up_thresh
            stem(times(t),log10(Effects(t)),'Color',nanmean(cols([1 2],:)),'MarkerEdgeColor',nanmean(cols([1 2],:)),'MarkerFaceColor','k','MarkerSize',marksize)
            g=g+1;
        elseif Effects(t)<up_thresh && Effects(t)>=down_thresh
            stem(times(t),log10(Effects(t)),'Color',[0 0 0],'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',[1 1 1],'MarkerSize',marksize)
        elseif Effects(t)<down_thresh
    ...
    for t=1:length(Effects)
        if Effects(t)>=up_thresh
            stem(times(t),log10(Effects(t)),'Color',nanmean(cols([2 3],:)),'MarkerEdgeColor',nanmean(cols([2 3],:)),'MarkerFaceColor','k','MarkerSize',marksize)
            g=g+1;
        elseif Effects(t)<up_thresh && Effects(t)>=down_thresh
            stem(times(t),log10(Effects(t)),'Color',[0 0 0],'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',[1 1 1],'MarkerSize',marksize)
        elseif Effects(t)<down_thresh
...
%% Combined time scales decoding
close all
clearvars -except accuracies titles times
min_y=0.48;
max_y=0.66;
start_span=50;
end_span=350;
...
for i=1:4
    ...
    for subj=1:size(data,1)
        [~,~,conts]=unique(scale_max_subj(subj,times>0));
        norm_data(subj,i,:) = accumarray(conts,1)./sum(accumarray(conts,1));
    end
    ...
    if colouring==0
        ...
        %         shadedErrorBar(times,dec_max,data_stded./sqrt(size(data,1)),'lineprops',{'k','markerfacecolor','k','LineWidth',2},'transparent',1);
        %         ylim([min_y max_y])
        %         xlim([-100 500])
        %         hold on;
        %         plot([-100 500],[0.5 0.5],'--k');
        %         plot([0 0],[min_y max_y],'--k');
        %         plot([times(time_max_1) times(time_max_1)],[min_y max_y],'--r')
        %         plot([times(time_max_2) times(time_max_2)],[min_y max_y],'--r')
        %         grid on
        %         title(titles{i});
        %
        %         subplot(212)
        %         plot(times,scale_max,'k','linewidth',2);
        %         hold on;
        %         plot(times,smoothdata(scale_max,'movmean',25),'r','linewidth',2);
        %         plot([times(time_max_1) times(time_max_1)],[1 10],'--r')
        %         plot([times(time_max_2) times(time_max_2)],[1 10],'--r')
        %         yticks([1:10])
        %         yticklabels([num2str(scales')])
        %         xlabel('Time (ms)')
...
ys=[0 0 0.3 0.3];
for t=1:256-1
    fill(xs([t t+1 t+1 t]),ys,cl(t,:),'EdgeColor','none','facealpha',0.5);
    hold on;
end
scales=[2 6 10 14 18 22 26 38 50 74];
...
for type=1:2
    ...
    for i=1:4
        ...
        for t=tics
            c=c+1;
            x=t.*ones(1,16);
            %             boxchart(x,squeeze(data(:,i,c))','BoxFaceColor',cols(i,:),'MarkerColor',cols(i,:),'MarkerStyle','none','notch','on');
...
%% Decoding/time scale tuning curves
clc;
close all
...
for type=1:2
    ...
    titles={'Orientation','Frequency','Colour','Contrast'};
    % cl=jet;
    % indx_cols=round(linspace(1,size(cl,1),length(titles)));
    % cols=cl(indx_cols,:);
    
    % cl=hsv;
    % cols=cl([30 80 150 220],:);
    ...
    windows={'Whole(0:+500ms)','Feed-forward(0:+160ms)','Recurrent(+160:+500ms)'};
    % for time_window=1:length(window_to_analyse)
    if type==1
        time_window= window_to_analyse{2};
    elseif type==2
        time_window= window_to_analyse{1};
...
%% decoding per condition for supplementary
clc
close all
min_y=0.48;
max_y=0.72;
